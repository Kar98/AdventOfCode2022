...
function () {
  ...
  Monkey.prototype.inspect_part1 = function () {
    ...
    for (var item_idx = 0; item_idx < this.items.length; item_idx++) {
      ...
      actions.push({
        monkey_to_throw_to: to_monkey,
        item_value: this.items[item_idx]
      });
  ...
  Monkey.prototype.throw_to_monkey = function (item_level) {
    if (item_level % this.test == 0) {
...
function print_items_inspected(monkeys) {
  for (var i = 0; i < monkeys.length; i++) {
    console.log("Monkey ".concat(i, " inspected items ").concat(monkeys[i].items_inspected, " times"));
  }
}

function setup_test_monkeys() {
  var monkeys = [];
  monkeys.push(new Monkey(0, [79, 98], function (i) {
    return i * 19;
  }, 23, 2, 3));
  monkeys.push(new Monkey(1, [56, 65, 75, 74], function (i) {
    return i + 6;
  }, 19, 2, 0));
  monkeys.push(new Monkey(2, [79, 60, 97], function (i) {
    return i * i;
  }, 13, 1, 3));
  monkeys.push(new Monkey(3, [74], function (i) {
    return i + 3;
  }, 17, 0, 1));
  return monkeys;
}
...
function setup_monkeys() {
  var monkeys = [];
  monkeys.push(new Monkey(0, [83, 97, 95, 67], function (i) {
    return i * 19;
  }, 17, 2, 7));
  monkeys.push(new Monkey(1, [71, 70, 79, 88, 56, 70], function (i) {
    return i + 2;
  }, 19, 7, 0));
  monkeys.push(new Monkey(2, [98, 51, 51, 63, 80, 85, 84, 95], function (i) {
    return i + 7;
  }, 7, 4, 3));
  monkeys.push(new Monkey(3, [77, 90, 82, 80, 79], function (i) {
    return i + 1;
  }, 11, 6, 4));
  monkeys.push(new Monkey(4, [68], function (i) {
    return i * 5;
  }, 13, 6, 5));
  monkeys.push(new Monkey(5, [60, 94], function (i) {
    return i + 5;
  }, 3, 1, 0));
  monkeys.push(new Monkey(6, [81, 51, 85], function (i) {
    return i * i;
  }, 5, 5, 1));
  monkeys.push(new Monkey(7, [98, 81, 63, 65, 84, 71, 84], function (i) {
    return i + 3;
  }, 2, 2, 3));
  return monkeys;
}
...
function part1() {
  ...
  var rounds = 20; // Each monkey takes turns inspecting all the items. Done in order from 0 to nth.
  ...
  for (var r = 1; r < rounds + 1; r++) {
    ...
    console.log("Round ".concat(r, " done"));
  ...
  console.log("Total monkeys : ".concat(monkeys.length));
  inspected = inspected.sort(function (a, b) {
    return b - a;
  });
  console.log("First monkey = ".concat(inspected[0], " . Second monkey = ").concat(inspected[1]));
  console.log("Monkey business = ".concat(inspected[0] * inspected[1]));
}

function part2() {
  var monkeys = setup_test_monkeys();
  var rounds = 500; // Each monkey takes turns inspecting all the items. Done in order from 0 to nth.
  // Monkey inspects, after it inspects, then current worry level / 3, then Math.floor()
  // Check worry level, then throw to other monkey
